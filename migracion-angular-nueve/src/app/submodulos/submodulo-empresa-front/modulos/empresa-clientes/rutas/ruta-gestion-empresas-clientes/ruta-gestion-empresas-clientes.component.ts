...
import { WIDTH_MODAL_EMPRESA_CLIENTE } from '../../constantes/tamanio-modal-empresa-cliente';
...
@Component({
  selector: 'ml-ruta-gestion-empresas-clientes',
  templateUrl: './ruta-gestion-empresas-clientes.component.html',
  styleUrls: ['./ruta-gestion-empresas-clientes.component.sass'],
})
export class RutaGestionEmpresasClientesComponent
  ...
  constructor(
    ...
    protected _empresaClientesRestService: EmpresaClientesRestService,
  ...
  ngOnInit() {
    ...
    this._activatedRoute.params.subscribe(
      params => {
        ...
        this.ruta = RUTAS_EMPRESA_CLIENTES.rutaGestionEmpresaClientes(
          true,
          false,
          [this.idEmpresa],
        );
        ...
        this.queryParams.relations =
          this.queryParams.relations.length > 0
            ? this.queryParams.relations
            : ['empresaCliente', 'empresaCliente.edificios'];
  ...
  abrirModalCrearEmpresaCliente() {
    const dialogRef = this.dialog.open(CrearEditarEmpresaClientesComponent, {
      width: WIDTH_MODAL_EMPRESA_CLIENTE,
      data: { idEmpresa: this.idEmpresa },
    });

    const resultadoModal$ = dialogRef.afterClosed();
    resultadoModal$.subscribe(
      (registroCreado: EmpresaClientesInterface) => {
        if (registroCreado) {
  ...
  abrirModalEditarEmpresaCliente(registro) {
    const indiceRegistro = this.values.indexOf(registro);
    const dialogRef = this.dialog.open(CrearEditarEmpresaClientesComponent, {
      width: WIDTH_MODAL_EMPRESA_CLIENTE,
      data: { empresaClientes: registro, idEmpresa: this.idEmpresa },
    });
    dialogRef.afterClosed().subscribe(
      (registroEditado: EmpresaClientesInterface) => {
        if (registroEditado) {
  ...
  busquedaPersonalizada(): void {
    ...
    this._empresaClientesRestService
      .obtenerEmpresasClientePorRazonSocialRUC(datos)
      .subscribe(
        resultado => {
          this._cargandoService.deshabilitarCargando();
  ...
  irGestionEdificios(idEmpresaClientes: number, gestionHijo) {
    const ruta = [
      'empresa-modulo',
      this.idEmpresa,
      'empresa-clientes-modulo',
      'gestion-empresas-clientes',
      idEmpresaClientes,
      'gestion-edificios-cliente',
    ];
    this._router.navigate(ruta);
  }
}
