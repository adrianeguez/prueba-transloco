...
@Component({
  selector: 'ml-carga-masiva-id-tipo-impuesto',
  templateUrl: './carga-masiva-id-tipo-impuesto.component.html',
  styleUrls: ['./carga-masiva-id-tipo-impuesto.component.scss'],
})
export class CargaMasivaIdTipoImpuestoComponent
  ...
  abrirModal() {
    const ventanaModal = this.dialog.open(ModalListaTipoImpuestoComponent, {
      width: '600px',
    });
  ...
  subirArchivoId() {
    const archivoCreado$ = this._guardarArchivoRestService.subirArchivo(
      ...
      this.tipoImpuestoSeleccionado.id,
    );
