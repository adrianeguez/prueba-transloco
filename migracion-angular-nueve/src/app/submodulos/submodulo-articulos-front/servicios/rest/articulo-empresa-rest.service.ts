...
export class ArticulosEmpresaRestService extends PrincipalRestService<
...
> {
  ...
  agregarArticulosPorEmpresa(datos) {
    return this._http.post(
      `${environment.url}:${environment.port}/${this.segmento}/agregar-articulos-empresa`,
      datos,
    );
  }

  obtenerArticuloIdEmpresa(datos) {
    return this._http.get(
      `${environment.url}:${environment.port}/${this.segmento}/obtener-articulo-por-empresa?empresa=${datos.idEmpresa}`,
      datos,
    );
  }
}
